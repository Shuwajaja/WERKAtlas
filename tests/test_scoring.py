...
def test_log_stars_logarithmic():
    ...
    # Going from 0 to 100 should be a bigger jump than from 10000 to 10100
...
def test_entry_scores_in_range():
    ...
    assert 0 <= score_adoption(entry) <= 10
    ...
    assert 0 <= score_community(entry) <= 5
    assert 0 <= score_uniqueness(entry) <= 5
...
def test_total_score_range():
    ...
    entry = {"name": "test", "description": "test", "topics": [], "stars": 500,
    ...
    total = (score_relevance(entry) + score_maintenance(entry) + score_adoption(entry) +
...
def test_archived_penalty():
    ...
    assert score_maintenance(archived) == 0
    assert score_maintenance(active) > 0
