...
- Name: brand.spec.ts >> Feature 4: WERK Branding & CSS Design Tokens >> Tier 4: Custom styles injected via query params does not compromise layout integrity
- Location: brand.spec.ts:119:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:4321/WERKAtlas/?theme=invalid_theme_value
Call log:
  - navigating to "http://localhost:4321/WERKAtlas/?theme=invalid_theme_value", waiting until "load"
...
> 121 |     await page.goto('/WERKAtlas/?theme=invalid_theme_value');
      |                ^ Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:4321/WERKAtlas/?theme=invalid_theme_value
  122 |     
  123 |     const body = page.locator('body');
  124 |     await expect(body).toBeVisible();
  125 |   });
  126 | 
  127 |   test('Tier 4: Focus outline styling is visible on keyboard navigation focus', async ({ page }) => {
  128 |     await page.goto('/WERKAtlas/');
  129 |     
  130 |     // Press Tab to focus search input
  131 |     await page.keyboard.press('Tab');
  132 |     const focusedInput = page.locator('[data-testid="search-input"]');
  133 |     
  134 |     const outline = await focusedInput.evaluate((el) => window.getComputedStyle(el).outlineStyle);
  135 |     // Ensure there is some visual focus indicator for accessibility compliance
  136 |     expect(outline).not.toBe('none');
  137 |   });
  138 | });
  139 | 
```