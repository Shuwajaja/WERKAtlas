...
- Name: explorer.spec.ts >> Feature 1: Atlas Explorer Search & Filters >> Tier 3: Deep-linking of filter criteria via query parameters
- Location: explorer.spec.ts:124:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:4321/WERKAtlas/?q=ollama&cat=models-infra
Call log:
  - navigating to "http://localhost:4321/WERKAtlas/?q=ollama&cat=models-infra", waiting until "load"
...
> 126 |     await page.goto('/WERKAtlas/?q=ollama&cat=models-infra');
  ...
  147 |   });
  148 | 
  149 |   test('Tier 4: Extreme pagination / infinite scroll stability', async ({ page }) => {
  150 |     await page.goto('/WERKAtlas/');
  151 |     // Scroll down multiple times
  152 |     await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  153 |     await page.waitForTimeout(500);
  154 |     await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  155 |     
  156 |     const cards = page.locator('[data-testid="project-card"]');
  157 |     expect(await cards.count()).toBeGreaterThan(0);
  158 |   });
  159 | });
  160 | 
```